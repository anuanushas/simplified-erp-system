...
const Dashboard = () => {
  const { products, ordersData } = useContext(Context);
  ...
  let ordersDataCounts = {};
  ordersData.forEach((order) => {
    let status = order.status;
    if (ordersDataCounts[status]) {
      ordersDataCounts[status]++;
    } else {
      ordersDataCounts[status] = 1;
    }
  });

  // Convert the dictionary to the desired array format
  let ordersDataArray = Object.keys(ordersDataCounts).map((key) => ({
    status: key,
    Count: ordersDataCounts[key],
  }));
      ...
      <div className="row">
        ...
        <div className="col-sm-6">
          <div className="card">
            ...
            <div className="card-body">
              {ordersDataArray.map((order, index) => {
                return (
                  <p
                    key={index}
              ...
              <h5 className="card-title">Total Orders: {ordersData.length}</h5>
